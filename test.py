...
def plot_results(clusters):
    ...
    good = [
        clusters[c]["хорошая"] / sum(clusters[c].values()) * 100 for c in cluster_names
    ]
    no_data = [
        clusters[c]["нет записи"] / sum(clusters[c].values()) * 100
        for c in cluster_names
    ]
    junk = [
        clusters[c]["мусор"] / sum(clusters[c].values()) * 100 for c in cluster_names
    ]
    failed = [
        clusters[c]["не получилось"] / sum(clusters[c].values()) * 100
        for c in cluster_names
    ]
    ...
    plt.barh(
        ...
        junk,
        color="red",
        left=[i + j for i, j in zip(good, no_data)],
        label="мусор",
    )
    plt.barh(
        cluster_names,
        failed,
        color="yellow",
        left=[i + j + k for i, j, k in zip(good, no_data, junk)],
        label="не получилось",
    )
...
def write_results_to_file(clusters, base_filename):
    ...
    cluster_keys = [int(k) for k in clusters.keys() if k != "all"]
    cluster_range = f"{min(cluster_keys)}-{max(cluster_keys)}"
    output_filename = f"{base_filename}_{timestamp}_{cluster_range}.txt"

    with open(output_filename, "w") as file:
        for cluster, data in clusters.items():
            if cluster != "all":  # Не записываем общий итог в файл
